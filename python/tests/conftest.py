...
async def http_client_no_preload():
    ...
    app = create_app(model_name="cl-nagoya/ruri-v3-30m", preload=False, idle_timeout=600)
...
async def http_client_with_preload(mock_sentence_transformer):
    ...
    from app.main import create_app
    app = create_app(model_name="cl-nagoya/ruri-v3-30m", preload=True, idle_timeout=600)
    async with LifespanManager(app):
        async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
